...
export const settingsSchema = z.strictObject({ name: text, strict: z.boolean(), unit, startDate: date, increasePercent: z.number().min(2).max(5), decreasePercent: z.number().min(2).max(3), restSeconds: z.number().int().min(0).max(86400), theme: z.enum(['light', 'dark', 'automatic']).optional(), checkInDay: z.number().int().min(0).max(6).optional() });
...
export const checkInSchema = z.strictObject({ id: idSchema, cycleId: idSchema.optional(), date, week: z.number().int().min(1).max(100_000), poorSleep: z.boolean(), runDown: z.boolean(), elevatedHr: z.boolean(), lingeringSoreness: z.boolean(), performanceDip: z.boolean(), jointPain: z.boolean(), notes: text, sleepHours: z.number().min(0).max(24).optional(), fatigue: z.number().int().min(1).max(5).optional(), soreness: z.number().int().min(1).max(5).optional(), stress: z.number().int().min(1).max(5).optional(), measuredPerformanceDip: z.boolean().optional(), performanceEvidence: z.array(text).max(100).optional(), assessmentVersion: z.literal('1').optional() });
