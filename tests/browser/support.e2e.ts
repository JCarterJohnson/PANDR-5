...
async function supportFixture(page:Page, total=7) {
  ...
  await page.route(`${PUBLIC_CLOUD.url}/rest/v1/**`, async route=>{
    ...
    if(path.endsWith('pandr_support_status'))return route.fulfill({json:[{total:total+Number(voted),voted}]});
...
test('heart persists on reload, toggles once, recovers an uncertain write and stays private after sign-out',async({page})=>{
  ...
  await expect(heart.getByRole('status')).toHaveText('7');
  await expect(heart.getByRole('status')).toHaveAccessibleName('7 upvotes');
  ...
  expect(f.writes()).toBe(1);await expect(panel.getByRole('status')).toHaveText('8');
  await panel.screenshot({path:'/tmp/pandr-heart-desktop.png'});
  ...
  await expect(panel.getByRole('status')).toHaveText('7');await expect(heart).toHaveAttribute('aria-pressed','false');
...
test('mobile heart reports unavailable counts honestly and retries',async({page})=>{
  ...
  await accountFixture(page);const f=await supportFixture(page,12567);f.setFailure(true);await page.goto('/');
  await page.getByRole('button',{name:'Open navigation',exact:true}).click();await settings(page);
  const panel=page.getByRole('region',{name:'Support PANDR-5'});
  await expect(panel.getByRole('status')).toHaveText('—');await expect(panel.getByRole('status')).toHaveAccessibleName('Count unavailable');await expect(panel.getByRole('button',{name:'Upvote PANDR-5',exact:true})).toBeDisabled();
  f.setFailure(false);await panel.getByRole('button',{name:'Retry upvotes'}).click();await expect(panel.getByRole('button',{name:'Upvote PANDR-5',exact:true}).getByRole('status')).toHaveText('12,567');
  await panel.scrollIntoViewIfNeeded();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
  await panel.screenshot({path:'/tmp/pandr-heart-mobile.png'});
});
